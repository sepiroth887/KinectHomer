...
import uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntervalEnumeration;
...
import uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.set.core.SetDomain;
...
// 		for (Var v : vars) {
	...
	public boolean leftChild(T var, int value, boolean status) {
		...
		if (!status && returnCode) {
			...
			if (previousSearchNode.dom instanceof uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntDomain)
				currentSearchNode.dom = ((IntDomain)previousSearchNode.dom).subtract( value );
			else if (previousSearchNode.dom instanceof uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.set.core.SetDomain)
...
//				currentSearchNode.dom = ((SetDomain)previousSearchNode.dom).subtract( value, value );
	...
	private int minValue(ArrayList<Var> vars) {
		int min = uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntDomain.MaxInt;
		if (vars.get(0) instanceof uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntVar)
			for (Var v : vars)
	...
	private int maxValue(ArrayList<Var> vars) {
		int max = uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntDomain.MinInt;
		if (vars.get(0) instanceof uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntVar)
			for (Var v : vars)
		...
		try {
			...
			if (tracedVar.size() != 0) {
				...
				for (int i = 0; i < tracedVar.size(); i++) {
					...
					if (tracedVar.get(i) instanceof uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core.IntVar) {

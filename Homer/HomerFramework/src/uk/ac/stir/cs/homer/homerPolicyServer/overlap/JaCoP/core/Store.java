...
package uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.core;
...
import uk.ac.stir.cs.homer.homerPolicyServer.overlap.JaCoP.util.SparseSet;
...
public class Store {
	...
	public static ArrayList<Var> variablesToTrace = new ArrayList<Var>();
	...
	static public void addTracedVariable(Var V) {
	...
	static public void removeTracedVariable(Var V) {
...
//		for (int j = pruningEvent; j <= IntDomain.ANY; j++) {
				...
				if (changed[c.queueIndex].add(c)) {
					...
					if (Switches.trace)
						if (Switches.traceStore)
							if (Switches.traceQueueingConstraint)
								System.out.println("Constraint " + c
										+ " has been touched by variable " + var
					...
					if (Switches.traceIndexicals)
						if (Switches.traceFailedConstraint)
							if (constraintsToTrace.contains(currentConstraint))
								System.out.println("\n*** Constraint failed :" + currentConstraint);
		...
		if (Switches.trace)
			if (Switches.traceStore)
				if (Switches.traceVariableCreation)
					System.out.println("Variable " + var + " has been created");

...
//			return;
		...
		if (variable == xVar && arc.index != Network.DELETED_ARC) {
		...
		// Weight variable was bounded
			...
			if (arc.cost != newCost) {
				...
				if (arc.index == Network.DELETED_ARC) {
...
//			assert (!xVar.singleton()) : " " + xVar + ", " + xVar.domain;
	...
	public int compareTo(ArcCompanion that) {
		if (this.arc.index == Network.DELETED_ARC)  {
			if (that.arc.index != Network.DELETED_ARC)
				return 1;
		} else {
			if (that.arc.index == Network.DELETED_ARC)
				return -1;

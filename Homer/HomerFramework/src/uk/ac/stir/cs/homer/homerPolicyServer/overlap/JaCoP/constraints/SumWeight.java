...
public class SumWeight extends Constraint {
	...
	public void consistency(Store store) {
		...
		if (backtrackHasOccured) {
			...
			for (int i = pointer; i < list.length; i++) {
				...
				int mul1 = multiply(currentDomain.min(), weights[i]);
				int mul2 = multiply(currentDomain.max(), weights[i]);
	...
	public void queueVariable(int level, Var var) {
		...
		if (var.singleton()) {
			...
			int mul1 = multiply(((IntVar)var).min(), weights[i]);
			int mul2 = multiply(((IntVar)var).max(), weights[i]);
